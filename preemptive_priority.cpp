...
void calc(int burst[], int arr[], int pr[], int n){
	...
	for (int time=1; count!=n; time++){				//iterated until all processes are completed
		smallest=n;
		for (int i=0; i<n; i++){
			if (arr[i]<=time && pr[i]<pr[smallest] && rem_burst[i]>0){
				smallest = i;
			}
		}
	...
	//display colums
	cout<<endl;
	cout<<"P\tBT\tAT\tPr\tWT\tTAT"<<endl;
	for (int i=0; i<n; i++){
		cout<<"P"<<i+1<<"\t"<<burst[i]<<"\t"<<arr[i]<<"\t"<<pr[i]<<"\t"<<WT[i]<<"\t"<<TAT[i];
		cout<<endl;
	}
